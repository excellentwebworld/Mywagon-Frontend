 ...
 *
 * Two-step flow:
 * 1. Choose: Download template OR Upload file (+ accepted values reference)
...
export default function ImportModal({ open, onClose, onImported, existingLanes }) {
  ...
  const handleFile = useCallback((e) => {
    ...
    reader.onload = (ev) => {
      const result = parseCsvText(String(ev.target?.result || ''), existingLanes);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 200 }}>
      ...
      <div className="relative w-full max-w-[640px] max-h-[80vh] flex flex-col rounded-2xl shadow-2xl"
          ...
          {!preview ? (
            <>
              ...
              <div className="mt-4 rounded-xl" style={{ border: `1px solid ${T.bd}` }}>
                ...
                >
                  <span>{t('priceLists.import.acceptedValues', 'Accepted values reference')}</span>
                    ...
                    {preview.rows.map((r, i) => {
                      ...
                      return (
                        <tr key={i} style={{ background: isErr ? 'rgba(239,68,68,0.05)' : 'transparent' }}>
                          ...
                          <td className="px-2 py-1" style={{ color: T.t1 }}>{r.oRaw}{!r.validO ? ' ⚠' : ''}</td>
                          <td className="px-2 py-1" style={{ color: T.t1 }}>{r.dRaw}{!r.validD ? ' ⚠' : ''}</td>
